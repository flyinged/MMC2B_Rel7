...
{
    dp_init_vars();        
    if ((hardware_interface != IAP_SEL )) // && (hardware_interface != GPIO_SEL)) //ML84 change
    {
        #ifdef ENABLE_DEBUG
...
void dp_exit(void)
{
    ...
    {
        dp_display_text("dp_exit():dp_exit_access_nvm()...");
        dp_exit_access_nvm();
        dp_display_text("DONE\n\r");
    }
#endif
//    NVIC_SystemReset();
//    while( 1 )
...
void dp_exit(void)
{
    ...
    {
        dp_display_text("DBGx4: dp_exit_access_nvm()\n\r"); //_MYDEBUG x4
        dp_exit_access_nvm();
    }
#endif
    opcode = BYPASS;
    dp_display_text("DBGx5: IRSCAN_out() instruction BYPASS (0xFF)\n\r"); //_MYDEBUG x5
    IRSCAN_out(&global_uchar);
    /* This is needed to avoid a potential glitch while exiting programming mode. */
    if (!(global_uchar & 0x4U))
    {
        dp_display_text("\n\rDBGx6: bypass output bit 2 is low\n\r"); //_MYDEBUG x6
        opcode = HIGHZ;
        dp_display_text("\n\rDBGx7: IRSCAN_in() instruction HIGHZ (0x07)\n\r"); //_MYDEBUG x6
        IRSCAN_in();
        dp_display_text("\n\rDBGx8: goto state JTAG_RUN_TEST_IDLE (1)\n\r"); //_MYDEBUG x6
        goto_jtag_state(JTAG_RUN_TEST_IDLE,HIGHZ_CYCLES);
        opcode = BYPASS;
        dp_display_text("\n\rDBGx9: IRSCAN_in() instruction BYPASS (0x07)\n\r"); //_MYDEBUG x6
        IRSCAN_in();
    }
    dp_display_text("\n\rDBGxA: goto state JTAG_RUN_TEST_IDLE (0)\n\r"); //_MYDEBUG x7
    goto_jtag_state(JTAG_RUN_TEST_IDLE,0u);
    dp_display_text("\n\rDBGxB: wait 288 us\n\r"); //_MYDEBUG x8
    dp_delay(BYPASS_DELAY);
    dp_display_text("\n\rDBGxC: goto state JTAG_TEST_LOGIC_RESET\n\r"); //_MYDEBUG x9
    goto_jtag_state(JTAG_TEST_LOGIC_RESET,0u);
