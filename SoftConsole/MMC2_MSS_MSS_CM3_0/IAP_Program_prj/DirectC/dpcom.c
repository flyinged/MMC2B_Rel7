...
uint32_t ADDR_OFFSET = 0x0000000u;
/*
...
extern uint32_t SPI_IMAGE_ADDR; //ML84 added
...
void dp_get_page_data(DPULONG image_requested_address)
{
    ...
    */
    ...
    /* get the data from where it is stored from */
    /* Need to get it starting from image_requested_address, place the content in 
          page_global_buffer and set return_bytes to the valid number of bytes returned */
    //#error "Please add code here to get the programming data. Please refer to the Required Source Code Modifications section of the DirectC user's guide."
#ifdef NEW_SPI_FLASH_DRIVER
    //FLASH_read(image_requested_address, page_global_buffer, return_bytes);
    FLASH_read(image_requested_address+SPI_IMAGE_ADDR, page_global_buffer, return_bytes); //ML84 added
#else
