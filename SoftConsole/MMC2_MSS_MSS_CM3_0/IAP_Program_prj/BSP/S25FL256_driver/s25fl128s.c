...
#define SPI_INSTANCE &g_mss_spi1 //ADDED: SPI FLASH CONNECTED TO SPI1, SLAVE0, MODE0, DIV_2, 8 but frame
#define SPI_SLAVE    MSS_SPI_SLAVE_0 //ADDED
...
void FLASH_init(void)
{
    ...
    MSS_SPI_configure_master_mode(SPI_INSTANCE,
                                   MSS_SPI_SLAVE_0,
                                   MSS_SPI_MODE3,
                                   MSS_SPI_PCLK_DIV_2, //25MHz/2 (could go faster but FAN controller's I2C speed is the limiting factor)
...
)
{
    ...
    *device_id = read_buffer[2];
    ...
    MSS_SPI_transfer_block(SPI_INSTANCE, dummy, 1, read_buffer, 2);
    MSS_SPI_clear_slave_select(SPI_INSTANCE, MSS_SPI_SLAVE_0);
...
/*******************************************************************************
 * FJ Invented to erase 256kB sector
 */
