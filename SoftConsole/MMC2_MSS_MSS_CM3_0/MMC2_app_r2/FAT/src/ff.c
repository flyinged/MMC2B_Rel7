...
#define	SS(fs)	512U
#elif _MAX_SS == 1024 || _MAX_SS == 2048 || _MAX_SS == 4096	|| _MAX_SS == 65536/* Multiple sector size */
...
)
{
	...
	if (vol >= _DRIVES)				/* Check if the drive number is valid */
		return FR_INVALID_DRIVE;
	rfs = FatFs[vol];				/* Get current fs object (static) */
...
)
{
	...
	DIR dj;
	BYTE *dir, n;
	DWORD dsc, dcl, pcl, tim = MSS_RTC_get_rtc_count();
	DEF_NAMEBUF;
...
//	DWORD n_vol, n_rsv, n_fat, n_dir;		/* Area size */
	UINT b_vol, b_fat, b_dir, b_data;		/* Area offset (LBA) */
	UINT n_vol, n_rsv, n_fat, n_dir;		/* Area size */
	...
	if (_MAX_SS != 512 && au < SS(fs)) au = SS(fs); //when sectors are bigger than 512, au= fs sector size
	au /= SS(fs);		/* Number of sectors per cluster */
	if (au == 0) au = 1; //minimum 1 sector
	if (au > 128) au = 128; //maximum 128 sectors

	/* Pre-compute number of clusters and FAT syb-type */
	n_clst = n_vol / au; //1 cluster contains AU sectors
	fmt = FS_FAT12;
